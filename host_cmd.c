// Baker: sv_cheats to 1, "map" close console
...
int current_skill;
cvar_t sv_cheats = {0, "sv_cheats", "1", "enables cheat commands in any game, and cheat impulses in dpmod [Zircon default]"}; // Baker 7002
...
static void Host_Status_f (void)
{
	...
	// status 2
	if (Cmd_Argc() == 2) {
		if (String_Does_Match (Cmd_Argv(1), "1") )
			in = 1;
		else if (String_Does_Match (Cmd_Argv(1), "2") )
			in = 2;
	}
	...
	if (in == 1)
		print ("32IP                                             %%pl ping  time   frags  no   name" NEWLINE);
	else if (in == 2)
		print ("^3IP                                              no   name" NEWLINE);
...
static void Host_God_f (void)
{
	...
	if (Cmd_Argc () > 1)	{ wants_on = atof(Cmd_Argv (1)) != 0; }
...
#define Console_Exit_If_Needed_()	\
...
static void Host_Map_f (void)
{
	...
	if (Cmd_Argc() != 2) {
		if (sv.active) {
			...
			if (cl.worldmodel)
				Con_DPrintLinef ("sv wateralpha support? %d", cl.worldmodel->brush.supportwateralpha);
		} else if (!sv.active && cls.state == ca_connected && cls.signon == SIGNONS) {
	...
	SV_SpawnServer(level, /*loadgame?*/ NULL) ;
...
static void Host_Restart_f (void)
{
	...
	SV_SpawnServer(mapname, /*loadgame?*/ NULL);
	if (sv.active && cls.state == ca_disconnected)
		CL_EstablishConnection("local:1", -2);
}
...
static void Host_Loadgame_f (void)
{
	...
	t = text = (char *)FS_LoadFile (filename, tempmempool, false, NULL, NOLOADINFO_IN_NULL, NOLOADINFO_OUT_NULL); // AXX1 START
	if (!text) {
	...
	if (developer_entityparsing.integer)
		Con_PrintLinef ("Host_Loadgame_f: loading version");
	...
	if (developer_entityparsing.integer)
		Con_PrintLinef ("Host_Loadgame_f: loading description");
...
// this silliness is so we can load 1.06 save files, which have float skill values
	...
	if(developer_entityparsing.integer)
		Con_PrintLinef ("Host_Loadgame_f: loading mapname");
	...
	if(developer_entityparsing.integer)
		Con_PrintLinef ("Host_Loadgame_f: loading time");
	...
	if(developer_entityparsing.integer)
		Con_PrintLinef ("Host_Loadgame_f: spawning server");

	// AXX1
	SV_SpawnServer (mapname, /*loadgame?*/ filename); // Precaches here!
	// AXX1 TOO LATE, sent to cl
	if (!sv.active)
	{
		Mem_Free(text);
	...
	if(developer_entityparsing.integer)
		Con_PrintLinef ("Host_Loadgame_f: loading light styles");
...
// load the light styles
	...
	if(developer_entityparsing.integer)
		Con_PrintLinef ("Host_Loadgame_f: skipping until globals");
...
// load the edicts out of the savegame file
	...
	for (;;)
	{
		...
		if (entnum == -1) {
			if (developer_entityparsing.integer)
				Con_PrintLinef ("Host_Loadgame_f: loading globals");
		...
		else
		{
			...
			if (entnum >= MAX_EDICTS) {
				Mem_Free(text);
				Host_Error("Host_PerformLoadGame: too many edicts in save file (reached MAX_EDICTS %d)", MAX_EDICTS);
			}
			...
			if(developer_entityparsing.integer)
				Con_PrintLinef ("Host_Loadgame_f: loading edict %d", entnum);
	...
	if(developer_entityparsing.integer)
		Con_PrintLinef ("Host_Loadgame_f: skipping until extended data");
	...
	if (end) {
		...
		if (end[0] == '/' && end[1] == '*' && (end[2] == '\r' || end[2] == '\n')) {
			if (developer_entityparsing.integer)
				Con_PrintLinef ("Host_Loadgame_f: loading extended data");
			...
			while (COM_ParseToken_Simple(&t, false, false, true)) {
				if (String_Does_Match(com_token, "sv.lightstyles")) {
					...
					if (i >= 0 && i < MAX_LIGHTSTYLES)
						...
						Con_PrintLinef ("unsupported lightstyle %d \"%s\"", i, com_token);
				}
					...
					if (i >= 0 && i < MAX_MODELS) {
						...
						Con_PrintLinef ("unsupported model %d \"%s\"", i, com_token);
				}
					...
					if (i >= 0 && i < MAX_SOUNDS)
						...
						Con_PrintLinef ("unsupported sound %d \"%s\"", i, com_token);
				}
					...
					if (COM_ParseToken_Simple(&t, false, false, true)) {
						...
						if (i >= 0) {
							...
							if (!BufStr_FindCreateReplace(prog, i, k, "string"))
								Con_PrintLinef ("failed to create stringbuffer %d", i);
						}
						else
							Con_PrintLinef  ("unsupported stringbuffer index %d \"%s\"", i, com_token);
					}
					else
				...
				else if (String_Does_Match(com_token, "sv.bufstr"))
				{
					...
					else
					{
						...
						if (stringbuffer)
						{
							...
							Con_PrintLinef ("failed to create stringbuffer %d \"%s\"", i, com_token);
					}
				}	
	...
	if(developer_entityparsing.integer)
		Con_PrintLinef ("Host_Loadgame_f: finished");
...
cvar_t nostartdemos = {CVAR_SAVE , "nostartdemos", "0", "Do not play start demos on startup [Zircon change]"};
...
cvar_t cl_rate = {CVAR_SAVE | CVAR_NQUSERINFOHACK, "_cl_rate", "30000", "internal storage cvar for current rate (changed by rate command) [Zircon Default mirror fteqw]"};
...
static void Host_User_f (void) // credit: taken from QuakeWorld
{
	...
	for (i = 0;i < cl.maxclients;i++)
	{
		if (!cl.scores[i].name[0])
			continue;
		if (cl.scores[i].qw_userid == uid || String_Does_Match_Caseless(cl.scores[i].name, Cmd_Argv(1)))
		{
...
void Host_InitCommands (void)
{
	...
	Cmd_AddCommand_WithClientCommand ("status", Host_Status_f, Host_Status_f, "print server status information. status 1 or 2 include ping");
	...
	Cmd_AddCommand ("startdemos", Host_Startdemos_f, "start playing back the selected demos sequentially (used at end of startup script)"); // cl_startdemos in Quakespasm
