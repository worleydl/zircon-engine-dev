...
cvar_t *Cvar_FindVar (const char *var_name)
{
	...
	for (var = cvar_hashtable[hashindex];var;var = var->nextonhashchain)
		if (String_Does_Match (var_name, var->name))
			return var;
...
============
*/
const char *Cvar_CompleteVariable (const char *partial, int is_from_nothing)
{
	cvar_t		*cvar;
	size_t		len;
...
*/
int Cvar_CompleteCountPossible (const char *partial, int is_from_nothing)
{
	cvar_t	*cvar;
	size_t	len;
	int		h;
	...
	for (cvar = cvar_vars; cvar; cvar = cvar->next) {
		if (String_Does_Start_With_Caseless (cvar->name, partial)) {
...
*/
const char **Cvar_CompleteBuildList (const char *partial, int is_from_nothing)
{
	const cvar_t *cvar;
	size_t len = 0;
	size_t bpos = 0;
	size_t sizeofbuf = (Cvar_CompleteCountPossible (partial, is_from_nothing) + 1) * sizeof (const char *);
	const char **buf;
	...
	for (cvar = cvar_vars; cvar; cvar = cvar->next)
		if (strncasecmp(partial, cvar->name, len) == 0)
			buf[bpos++] = cvar->name;
...
static void Cvar_SetQuick_Internal (cvar_t *var, const char *value)
{
	...
	else if ((var->flags & CVAR_NQUSERINFOHACK) && cls.state != ca_dedicated)
	{
		...
		if (String_Does_Match(var->name, "_cl_color"))
		{
...
#ifdef CONFIG_MENU
		else if (String_Does_Match(var->name, "net_slist_favorites"))
