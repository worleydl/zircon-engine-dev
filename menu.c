		...
		else
		{
			...
			for (j = 0;j < NUMKEYS;j++)
			{
				if (keys[j] != -1)
				{
					...
					strlcat(keystring, Key_KeynumToString (keys[j], tinystr, TINYSTR_LEN_4), sizeof(keystring));
				}
			}
		}
...
static void M_Keys_Key(cmd_state_t *cmd, int k, int ascii)
{
	...
	{	// defining a key
		...
		{
			c_dpsnprintf2 (line, "bind \"%s\" \"%s\"\n", Key_KeynumToString(k, tinystr, TINYSTR_LEN_4), bindnames[keys_cursor][0]);
