...
#include "snd_main.h"
	...
	int did_scaleauto_already;// = false;
	int shall_exclude;// = CLVM_prog && IS_NEXUIZ_DERIVED(gamemode) && csqc_full_width_height.integer < 2;
	int shall_set_full_window_size;// = CLVM_prog && csqc_full_width_height.integer && shall_exclude==false;
...
cvar_t csqc_full_width_height = {CVAR_SAVE, "csqc_full_width_height", "0", "csqc always gets a full client-sized canvas, vid_conwidth and vid_conheight are vid.width vid.height during csqc draws. 0 - Don't 1 - Yes [Zircon]"};
cvar_t csqc_full_width_height_available = {CVAR_READONLY, "csqc_full_width_height_available", "1", "Indicates ability for csqc to gain access full width/height resolution, otherwise a mod would want to set vid_conwidth in csqc like Xonotic [Zircon]"};
#ifdef CONFIG_MENU
...
cvar_t scr_loadingscreen_barcolor = {0, "scr_loadingscreen_barcolor", "0.25 0.25 0.25", "rgb color of loadingscreen progress bar [Zircon default]"};
...
cvar_t vid_conscale_auto = {CVAR_SAVE, "vid_conscale_auto", "1", "Forces vid_conwidth and vid_conheight to vid.width and vid.height before drawing a frame so CSQC gets full ortho [Zircon]"}; // Baker 4003
...
cvar_t tool_inspector = {0, "tool_inspector", "0", "view visible entity QC information [Zircon]"}; // Baker
cvar_t tool_show_position = {0, "tool_show_position", "0", "view player origin information [Zircon]"}; // Baker
cvar_t tool_show_angles = {0, "tool_show_angles", "0", "view player angle information [Zircon]"}; // Baker
...
//cvar_t scr_showpos = {CVAR_SAVE, "scr_showpos", "0", "shows position on screen"}; // Baker 1025
...
wrap3d:
	...
	// NZP uses a custom engine.
	if (shall_exclude == false) {
		if (shall_restore) { // RESTORE vid_conscale_auto
			Cvar_SetValueQuick (&vid_conwidth,  oldw);
...
cldraw2d3:
	...
	if (shall_exclude == false) {
		if (shall_set_full_window_size) {
			...
			// We exclude those.  csqc_full_width_height.integer 2 will force.
			// Will disable. csqc_full_width_height.integer 2
