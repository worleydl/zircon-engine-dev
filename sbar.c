// Baker: showfps_framerate >= 0.00001
...
cvar_t showfps = {CVAR_SAVE, "showfps", "0", "shows your rendered fps (frames per second) Set to -1 for top left [Zircon]"};
...
cvar_t sbar_q64 = {CVAR_SAVE, "sbar_q64", "0", "Draw the HUD similar to N64 Quake 64 hud. [Zircon]" };
cvar_t sbar_scorerank = {CVAR_SAVE, "sbar_scorerank", "0", "shows an overlay for your score (or team score) and rank in the scoreboard"}; // Baker 7006
cvar_t sbar_gametime = {CVAR_SAVE, "sbar_gametime", "0", "shows an overlay for the time left in the current match/level (or current game time if there is no timelimit set)"}; // Baker 7006
cvar_t scr_clock = {CVAR_SAVE, "scr_clock", "0", "shows game time in status bar [Zircon]"}; // Baker 7006
cvar_t sbar_showprotocol = {CVAR_SAVE, "sbar_showprotocol", "1", "shows client protocol in status bar [Zircon]"}; // Baker
...
cvar_t sbar_quake = {CVAR_SAVE, "sbar_quake", "0", "1: Draw Quake sbar backtile, 2: Quake Remaster/Quake 64 look (N64)  [Zircon]"};
...
cvar_t crosshair_color_red = {CVAR_SAVE, "crosshair_color_red", "0.5", "customizable crosshair color"};
cvar_t crosshair_color_green = {CVAR_SAVE, "crosshair_color_green", "0.5", "customizable crosshair color"};
cvar_t crosshair_color_blue = {CVAR_SAVE, "crosshair_color_blue", "0.5", "customizable crosshair color"};
cvar_t crosshair_color_alpha = {CVAR_SAVE, "crosshair_color_alpha", "0.9", "how opaque the crosshair should be"};
cvar_t crosshair_size = {CVAR_SAVE, "crosshair_size", "0.5", "adjusts size of the crosshair on the screen"};
...
static void sbar_start(void)
{
	...
	else
	{
		...
		sb_backtile = Draw_CachePic_Flags ("gfx/backtile", CACHEPICFLAG_QUIET);
...
static void Tool_Texturepointer_f(void)
{
	Cbuf_AddTextLine ("toggle showtex");
}
...
//	{
//		numdigits = -numdigits;
//		slen = dpsnprintf(str, sizeof(str), "%0*i", numdigits, num);
//	}
//	else
//		slen = dpsnprintf(str, sizeof(str), "%d", num);
//	ptr = str;
...
levtime:
	...
	if (1 || sb_showscores) {
		...
		Sbar_DrawString ((320 - 8 -3 ) - strlen(vabuf) * 8, -24, vabuf);
	}
...
static void Sbar_DrawInventory (void)
{
	...
	if (gamemode == GAME_ROGUE) {
		if ( cl.stats_sv[STAT_ACTIVEWEAPON] >= RIT_LAVA_NAILGUN )	Sbar_DrawAlphaPic (0, -24, rsb_invbar[0], sbar_alpha_bg.value);
		else													Sbar_DrawAlphaPic (0, -24, rsb_invbar[1], sbar_alpha_bg.value);
	}
	else
			...
			if (cl.stats_sv[STAT_ITEMS] & (1<<hipweapons[i]) ) {
				...
				if (i==2) {
					...
					if (cl.stats_sv[STAT_ITEMS] & (IT_SHOTGUN<<4)) {
						if (!grenadeflashing)	Sbar_DrawPic (96, -16, hsb_weapons[flashon][3]);
					}
...
void Sbar_ShowFPS(void)
{
	...
	texstring[0] = 0;
	topspeedstring[0] = 0;
	if (showfps.integer && showfps_framerate >= 0.00001 && cls.signon == SIGNONS)  // Baker 1027
	{
	...
	extern cvar_t tool_show_position;
	extern cvar_t tool_show_angles;
	if (tool_show_position.integer) {
		if (cl.entities) {
			dpsnprintf(posstring, sizeof(posstring), "%d %d %d", (int)cl.entities[cl.playerentity].state_current.origin[0], (int)cl.entities[cl.playerentity].state_current.origin[1], (int)cl.entities[cl.playerentity].state_current.origin[2]);
			fps_strings++;
		}
	}
	if (tool_show_angles.integer) {
		if (cl.entities) {
	...
	if (fps_strings || showfps.integer < 0)
	{
		...
		if (fpsstring[0])
		{
			...
			if (showfps.integer < 0) {
				...
				//DrawQ_TextWidth(fpsstring, 0, con_notifysize.value, con_notifysize.value, true, FONT_CENTERPRINT) - con_notifysize.value;

				//DrawQ_Fill(fps_x, fps_scaley * 2, vid_conwidth.integer - fps_x - fps_scalex * 2, fps_scaley, 0, 0, 0, 0.5, 0);
				//if (red)
				//	DrawQ_String(fps_x, fps_scaley * 2, fpsstring, /*maxlen*/ 0, fps_scalex, fps_scaley, /*rgba*/ 1, 0, 0, 1, /*flags*/ 0, NULL, /*ig color codes*/ true, FONT_INFOBAR);
				//else
					DrawQ_String(fps_x, fps_scaley * 2, fpsstring, /*maxlen*/ 0, fps_scalex, fps_scaley, /*rgba*/ 1, 1, 1, 1, /*flags*/ 0, NULL, /*ig color codes*/ true, FONT_INFOBAR);
...
//{
//			if (sb_showscores || (cl.stats_sv[STAT_HEALTH] <= 0 && cl_deathscoreboard.integer))
//			{
...
//				for (i = 0; i < 8;i++)
//					if (cl.stats_sv[STAT_ITEMS] & (1 << i))
//						Sbar_DrawWeapon(i + 1, fade, (i + 2 == cl.stats_sv[STAT_ACTIVEWEAPON]));
//				if((cl.stats_sv[STAT_ITEMS] & (1<<12)))
//					Sbar_DrawWeapon(0, fade, (cl.stats_sv[STAT_ACTIVEWEAPON] == 1));
//
//				// flag icons
//				redflag = ((cl.stats_sv[STAT_ITEMS]>>15) & 3);
//				blueflag = ((cl.stats_sv[STAT_ITEMS]>>17) & 3);
...
//
//				// armor
//				if (cl.stats_sv[STAT_ARMOR] > 0)
//				{
//					Sbar_DrawStretchPic (72, 0, sb_armor[0], sbar_alpha_fg.value, 24, 24);
//					if(cl.stats_sv[STAT_ARMOR] > 200)
//						Sbar_DrawXNum(0,0,cl.stats_sv[STAT_ARMOR],3,24,0,1,0,1,0);
//					else if(cl.stats_sv[STAT_ARMOR] > 100)
//						Sbar_DrawXNum(0,0,cl.stats_sv[STAT_ARMOR],3,24,0.2,1,0.2,1,0);
//					else if(cl.stats_sv[STAT_ARMOR] > 50)
//						Sbar_DrawXNum(0,0,cl.stats_sv[STAT_ARMOR],3,24,0.6,0.7,0.8,1,0);
//					else if(cl.stats_sv[STAT_ARMOR] > 25)
//						Sbar_DrawXNum(0,0,cl.stats_sv[STAT_ARMOR],3,24,1,1,0.2,1,0);
//					else
//						Sbar_DrawXNum(0,0,cl.stats_sv[STAT_ARMOR],3,24,0.7,0,0,1,0);
//				}
//
//				// health
//				if (cl.stats_sv[STAT_HEALTH] != 0)
//				{
//					Sbar_DrawStretchPic (184, 0, sb_health, sbar_alpha_fg.value, 24, 24);
//					if(cl.stats_sv[STAT_HEALTH] > 200)
//						Sbar_DrawXNum(112,0,cl.stats_sv[STAT_HEALTH],3,24,0,1,0,1,0);
//					else if(cl.stats_sv[STAT_HEALTH] > 100)
//						Sbar_DrawXNum(112,0,cl.stats_sv[STAT_HEALTH],3,24,0.2,1,0.2,1,0);
//					else if(cl.stats_sv[STAT_HEALTH] > 50)
//						Sbar_DrawXNum(112,0,cl.stats_sv[STAT_HEALTH],3,24,0.6,0.7,0.8,1,0);
//					else if(cl.stats_sv[STAT_HEALTH] > 25)
//						Sbar_DrawXNum(112,0,cl.stats_sv[STAT_HEALTH],3,24,1,1,0.2,1,0);
//					else
//						Sbar_DrawXNum(112,0,cl.stats_sv[STAT_HEALTH],3,24,0.7,0,0,1,0);
//				}
//
//				// ammo
//				if ((cl.stats_sv[STAT_ITEMS] & (NEX_IT_SHELLS | NEX_IT_BULLETS | NEX_IT_ROCKETS | NEX_IT_CELLS)) || cl.stats_sv[STAT_AMMO] != 0)
//				{
//					if (cl.stats_sv[STAT_ITEMS] & NEX_IT_SHELLS)
//						Sbar_DrawStretchPic (296, 0, sb_ammo[0], sbar_alpha_fg.value, 24, 24);
//					else if (cl.stats_sv[STAT_ITEMS] & NEX_IT_BULLETS)
//						Sbar_DrawStretchPic (296, 0, sb_ammo[1], sbar_alpha_fg.value, 24, 24);
//					else if (cl.stats_sv[STAT_ITEMS] & NEX_IT_ROCKETS)
//						Sbar_DrawStretchPic (296, 0, sb_ammo[2], sbar_alpha_fg.value, 24, 24);
//					else if (cl.stats_sv[STAT_ITEMS] & NEX_IT_CELLS)
//						Sbar_DrawStretchPic (296, 0, sb_ammo[3], sbar_alpha_fg.value, 24, 24);
//					if(cl.stats_sv[STAT_AMMO] > 10)
//						Sbar_DrawXNum(224, 0, cl.stats_sv[STAT_AMMO], 3, 24, 0.6,0.7,0.8,1,0);
//					else
//						Sbar_DrawXNum(224, 0, cl.stats_sv[STAT_AMMO], 3, 24, 0.7,0,0,1,0);
//				}
//
...
//
//				// armor
//				Sbar_DrawXNum ((340-3*24), 12, cl.stats_sv[STAT_ARMOR], 3, 24, 0.6,0.7,0.8,1,0);
//
//				// health
//				if(cl.stats_sv[STAT_HEALTH] > 100)
//					Sbar_DrawXNum((154-3*24),12,cl.stats_sv[STAT_HEALTH],3,24,1,1,1,1,0);
//				else if(cl.stats_sv[STAT_HEALTH] <= 25 && cl.time - (int)cl.time > 0.5)
//					Sbar_DrawXNum((154-3*24),12,cl.stats_sv[STAT_HEALTH],3,24,0.7,0,0,1,0);
//				else
//					Sbar_DrawXNum((154-3*24),12,cl.stats_sv[STAT_HEALTH],3,24,0.6,0.7,0.8,1,0);
//
//				// AK dont draw ammo for the laser
//				if(cl.stats_sv[STAT_ACTIVEWEAPON] != 12)
//				{
//					if (cl.stats_sv[STAT_ITEMS] & NEX_IT_SHELLS)
...
//
//					if(cl.stats_sv[STAT_AMMO] <= 10)
//						Sbar_DrawXNum ((519-3*24), 12, cl.stats_sv[STAT_AMMO], 3, 24, 0.7, 0,0,1,0);
//					else
//						Sbar_DrawXNum ((519-3*24), 12, cl.stats_sv[STAT_AMMO], 3, 24, 0.6, 0.7,0.8,1,0);
//
//				}
//
//				if (sb_lines > 24)
...
no_armor:
	...
	if (sb_showscores) {
		...
		//Sbar_SoloScoreboard (); // vad wy?
		Sbar_DrawScoreboard ();
	} else if (sb_showscores || (cl.stats_sv[STAT_HEALTH] <= 0 && cl_deathscoreboard.integer)) {
		// DEAD
		Sbar_DrawAlphaPic (0, 0, sb_scorebar, sbar_alpha_bg.value);
		Sbar_DrawScoreboard ();
	} else if (sb_lines) {
		...
		if (gamemode == GAME_HIPNOTIC || gamemode == GAME_QUOTH) {
			if (cl.stats_sv[STAT_ITEMS] & IT_KEY1)		Sbar_DrawPic (209, 3, sb_items[0]);
			if (cl.stats_sv[STAT_ITEMS] & IT_KEY2)		Sbar_DrawPic (209, 12, sb_items[1]);
		}
...
void Sbar_DeathmatchOverlay (void)
{
	...
	DrawQ_Pic ((vid_conwidth.integer - sb_ranking->width)/2, ymin /*8*/, sb_ranking, 0, 0, 1, 1, 1, 1 * sbar_alpha_fg.value, 0);
...
//	if (sbar_scorerank.integer>1 && me >= 0 && me < cl.maxclients)
//	{
//		if(Sbar_IsTeammatch())
//		{
//			// Layout:
...
//	}
//
//	if (sbar_gametime.integer > 1 && cl.statsf[STAT_TIMELIMIT])
//	{
...
//	}
//	else if (sbar_gametime.integer > 1)
//	{
...
// LA: Display as "a" instead of "a/b" if b is 0
	if (1 || cl.stats_sv[STAT_TOTALSECRETS])
	{
	...
	if (1 || cl.stats_sv[STAT_TOTALMONSTERS])
	{
